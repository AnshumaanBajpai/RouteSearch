...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		 //obtaining the first neigbours of v
		 List<Integer> f_neigh = getNeighbors(v);
		 
		 //Now we need to find a list of first neighbors of each of the above
		 List<Integer> s_neigh = new ArrayList<Integer>();
		 
		 for(int i: f_neigh){
			 List<Integer> f_f_neigh = getNeighbors(i);
			 for (int j: f_f_neigh){
				 if(!s_neigh.contains(j)){
					 s_neigh.add(j);
				 }
			 }
		}
